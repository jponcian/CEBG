...
//-----------------
$tabla = $_SESSION['conexionsql']->query($_SESSION['consulta1d']);
// ----------
if ($tabla->num_rows>0)	{
$pdf->AddPage();
...
//-----------------
$i=0; $monto=0;	$id=0; $id_orden=0;
while ($registro = $tabla->fetch_object())
	{
	$pdf->SetFont('Times','',8.5);
	if ($id<>$registro->id_solicitud)	
		{
		$i++;
		$pdf->SetFont('Times','B',8.5);
		$id = $registro->id_solicitud;
		$pdf->SetFillColor(235);
		//----------
		$pdf->Cell($aa,5.5,$i,1,0,'C',1);
		...
		$pdf->SetFont('Times','',8.5);
		//----------
		$pdf->Cell($aa+$a+$b-3,5.5,formato_categoriao($registro->categoria).' '.formato_partida($registro->partida),1,0,'C',1);
		$pdf->Cell($g=13,5.5,formato_moneda($registro->cantidad),1,0,'R',1);
		$pdf->SetFont('Times','',7);
		$pdf->Cell($h=100,5.5,substr($registro->descripcion,0,50),1,0,'L',1);
		$pdf->SetFont('Times','',8.5);
		$pdf->Cell(0,5.5,formato_moneda($registro->total),1,0,'R',1);
		$pdf->Ln(5.5);
		//-----------
		} 
	else 
...
//-----------------
$tabla = $_SESSION['conexionsql']->query($_SESSION['consulta2d']);
// ----------
if ($tabla->num_rows>0)	{
$pdf->AddPage();
$pdf->SetFillColor(2, 117, 216);
