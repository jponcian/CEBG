...
<form id="form1" name="form1" method="post" onsubmit="return evitar();">
	<div align="center" class="TituloP">Relaci&oacute;n de Ordenes de Compra</div>
	<br>
...
<script language="JavaScript">
	...
	function asignar_numero(id) {
		...
		$('#modal_n').load('compras/2_modal.php?id=' + id);
	}
	//------------------
	function generar_solicitud(id) {
		Swal.fire({
			title: "¿Estas seguro de realizar la Orden?",
			icon: "question",
			...
			if (result.isConfirmed) {
				...
				$.ajax({
					type: 'POST',
					url: 'compras/2j_guardar.php?id=' + document.form888.txt_nuevo.value,
					dataType: "json",
					data: parametros,
					success: function(data) {
						if (data.tipo == "info") {
							...
							buscar();
							window.open("compras/formatos/10_orden.php?p=1&id=" + data.id, "_blank");
							window.open("compras/formatos/8_recepcion.php?p=1&id=" + data.id, "_blank");
						} else {
	...
	function buscar2() {
		...
		$('#div1').load('compras/2a_tabla.php?valor=' + cambia(document.form1.obuscar.value) + '&tipo=3');
	}
	//----------------
	function buscar() {
		if ((document.form1.obuscar.value == "  " || document.form1.obuscar.value == " " || document.form1.obuscar.value == "") && document.form1.optradio.value < 3) {} else {
			...
			$('#div1').html('<div align="center"><div class="spinner-border" role="status"></div><br><strong>Un momento, por favor...</strong></div>');
			$('#div1').load('compras/2a_tabla.php?valor=' + cambia(document.form1.obuscar.value) + '&tipo=' + document.form1.optradio.value);
		}
	}
	//---------------------
	function imprimir(id) {
		window.open("compras/formatos/10_orden.php?p=0&id=" + id, "_blank");
	}
</script>