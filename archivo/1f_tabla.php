...
<tbody><?php 	
$dato_filtrar = trim($_GET['filtro']);
$filtro = $_GET['tipo'];
$filtrar = " ORDER BY grupo, abs(numero), descripcion";
if ($dato_filtrar<>'')
{switch ($filtro) {
    case 1:
        $filtrar = " AND grupo LIKE '%$dato_filtrar%' ORDER BY grupo, abs(numero), descripcion";
        break;
//    case 2:
...
<td ><div align="center" ><a data-toggle="tooltip" title="Editar"><button type="button" class="btn btn-outline-info light-3 btn-sm" data-toggle="modal" data-target="#modal_largo" onclick="basicos(<?php echo ($registro->id); ?>);" data-keyboard="false"><i class="far fa-edit"></i></button></a></div></td>
