...
//////// ---- DETALLE
$consulta_x = "SELECT bn_materiales.*, bn_categorias.codigo FROM bn_categorias, bn_materiales WHERE inventario>0 AND bn_materiales.id_categoria = bn_categorias.id_categoria $filtro ORDER BY bn_materiales.descripcion_bien";
...
while ($registro_x = $tabla_x->fetch_object())
	...
	$alto2 = $y2 - $y1;
	//---------------------------------------------------
	$pdf->Cell($a,($alto2),formato_moneda($registro_x->inventario),$linea,0,'R');
	...
	//---------------------
	$pdf->Ln($alto2);
	$_SESSION['i']+=$registro_x->inventario;
	}
