...
<td ><div align="center" ><input onFocus="this.select()" value="<?php echo ($registro->cant_aprobada); ?>" id="O<?php echo ($registro->id_detalle); ?>" name="O<?php echo ($registro->id_detalle); ?>" size="2" class="input-group-text yellow-input" type="text" onkeypress="agregar2(event,'<?php echo ($registro->id_detalle); ?>',this.value); return SoloNumero(event,this);" maxlength="10"></td>
</tr>
 <?php 
 }
 ?>
</table>
<div align="center"><button data-toggle="modal" data-target="#modal_largo" data-backdrop="static" data-keyboard="false" onclick="generar_solicitud('<?php echo encriptar($id); ?>');" type="button" class="btn btn-outline-success waves-effect"><i class="fas fa-cloud-upload-alt prefix grey-text mr-1"></i>Procesar</button></div>
