...
$x=$this->GetX();
$y=$this->GetY();
$this->multicell($b,6,'Código del Catalago',1,'C');
$this->SetY($y);
$this->SetX($x+$b);

$x=$this->GetX();
$y=$this->GetY();
$this->multicell($c,4,'Número de Inventario (solo para bienes)',1,'C');
$this->SetY($y);
$this->SetX($x+$c);

$this->cell(strtoupper($d),12,'DESCRIPCION',1,0,'C');

$this->cell($e,12,'Código',1,0,'C');

$this->cell($f,12,'Concepto',1,0,'C');

$x=$this->GetX();
$y=$this->GetY();
$this->multicell($g,6,'Estado de Conservación',1,'C');
$this->SetY($y);
$this->SetX($x+$g);
...
$x=$this->GetX();
$y=$this->GetY();
$this->multicell($e,6,'Estado de Conservación',1,'C');
$this->SetY($y);
$this->SetX($x+$e);
