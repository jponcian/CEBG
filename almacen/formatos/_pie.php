...
if ($_SESSION['tipo']==21 or $_SESSION['tipo']==121 or $_SESSION['tipo']==31 or $_SESSION['tipo']==131)
	...
	{
	//----------------------------
	$consulta_x = "SELECT * FROM rac WHERE  id_area = '6' AND jefe_area = '1' LIMIT 1;";
	$tabla_x = $_SESSION['conexionsql']->query($consulta_x);
	$registro_x = $tabla_x->fetch_object();
	//----------------------------
	
	$this->SetFont('Arial','B',$_SESSION['fuente_cabecera']); 
	$this->Cell($a+$b+$c,$alto_cabecera+2,'Preparado por:         '.$registro_x->nombre.'         C.I. V-'.formato_cedula($registro_x->cedula).'         COORDINADOR BIENES NACIONALES ',1,0,'L');
	}
?>