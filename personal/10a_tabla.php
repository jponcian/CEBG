...
switch ($filtro) {
    case 1:
        ...
        break;		
    case 4:
        $consultx = "SELECT rac.*, CONCAT(rac.nombre,' ',rac.nombre2,' ',rac.apellido,' ',rac.apellido2) as  nombreA FROM rrhh_permisos, rac WHERE rrhh_permisos.cedula = rac.cedula AND rrhh_permisos.tipo <> 'COMISION' AND rrhh_permisos.tipo <> 'VACACIONES' GROUP BY rac.cedula ORDER BY rrhh_permisos.fecha DESC";
        break;		
    case 6:
...
if ($filtro<>4) {$consultx = "SELECT *, CONCAT(rac.nombre,' ',rac.nombre2,' ',rac.apellido,' ',rac.apellido2) as nombreA FROM rac WHERE 1=1 $filtrar ORDER BY vacaciones, nomina, ubicacion, cedula;";}
//echo $consultx;
