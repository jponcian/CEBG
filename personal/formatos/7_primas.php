...
//-----------------
$quincena = $descripcion.' DE '.strtoupper($_SESSION['meses_anno'][intval(mes($desde))]).' '.anno($desde);
//-------------	
...
$pdf->MultiCell(0,5,"Preliminar Pago Tipo: $tipo_pago de la Nomina $nomina correspondiente a: $quincena, Desde: ".voltea_fecha($desde)." Hasta: ".voltea_fecha($hasta).".",0,'J');// ".rellena_cero($numero,3)."
$pdf->Ln(3);
...
$pdf->Cell($b=45,5,'Empleado',1,0,'C',1);
$pdf->Cell($c=15,5,'Ingreso',1,0,'C',1);
$pdf->Cell($d=50,5,'Cargo',1,0,'C',1);
$pdf->Cell($e=25,5,'Sueldo Mensual',1,0,'C',1);
$pdf->Cell($f=25,5,'Hogar',1,0,'C',1);
$pdf->Cell($f,5,'Representacion',1,0,'C',1);
$pdf->Cell($f,5,'Responsabilidad',1,0,'C',1);
//$pdf->Cell($g=12,5,'Dias',1,0,'C',1);
//$pdf->Cell($h=25,5,'Bono Vacacional',1,0,'C',1);
...
//		}
	...
	$pdf->Cell($f,$alto,formato_moneda($registro->hogar),1,0,'R',1);
	$pdf->Cell($f,$alto,formato_moneda($registro->representacion),1,0,'R',1);
	$pdf->Cell($f,$alto,formato_moneda($registro->responsabilidad),1,0,'R',1);
