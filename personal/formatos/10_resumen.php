...
//------------
$consultx = "SELECT *, num_nomina as numero FROM nomina WHERE desde = '$desde' AND tipo_pago='001' LIMIT 1;";
//echo $consultx;
...
//--------------
$consulta = "SELECT COUNT(cedula) FROM nomina WHERE desde = '$desde' AND tipo_pago='001' GROUP BY cedula;";  	
...
$pdf->MultiCell(0,5,"Preliminar Pago de la Nomina correspondiente a: $quincena, Desde: ".voltea_fecha($desde)." Hasta: ".voltea_fecha($hasta).".",0,'J');// ".rellena_cero($numero,3)."
$pdf->Ln(3);
...
$pdf->Cell($d=18,5,'Sueldo',1,0,'C',1);
$pdf->Cell($e=16,5,'Prof.',1,0,'C',1);
$pdf->Cell($f=16,5,'Antiguedad',1,0,'C',1);
...
$consulta = "SELECT nomina.*, CONCAT(rac.nombre,' ',rac.nombre2,' ',rac.apellido,' ',rac.apellido2) as  nombre, rac.ing_dia, rac.ing_mes, rac.ing_ano FROM nomina, rac WHERE nomina.tipo_pago='001' AND rac.cedula = nomina.cedula AND nomina.desde = '$desde' ORDER BY nomina.ubicacion, nomina.nomina, rac.cedula ASC;";
$tabla = $_SESSION['conexionsql']->query($consulta);
//-----------------
$z=0;
$monto = 0;
...
while ($registro = $tabla->fetch_object())
{
	...
	$pdf->Cell($d,$alto,formato_moneda($registro->sueldo),1,0,'R',1);
	$pdf->Cell($e,$alto,formato_moneda($registro->prof),1,0,'R',0);
	$pdf->Cell($f,$alto,formato_moneda($registro->antiguedad),1,0,'R',0);
	$pdf->Cell($g,$alto,formato_moneda($registro->hijos),1,0,'R',0);
	$pdf->Cell($h,$alto,formato_moneda($registro->bono),1,0,'R',0);
	$pdf->Cell($m,$alto,formato_moneda($tickets),1,0,'R',0);
	$pdf->Cell($i,$alto,formato_moneda($registro->asignaciones+$tickets),1,0,'R',0);
	$pdf->Cell($j,$alto,formato_moneda($registro->sso),1,0,'R',0);
	$pdf->Cell($j,$alto,formato_moneda($registro->fp),1,0,'R',0);
	$pdf->Cell($j,$alto,formato_moneda($registro->lph),1,0,'R',0);
	//$pdf->Cell($m,$alto,formato_moneda($registro->fusamieg),1,0,'R',0);
	$pdf->Cell($j,$alto,formato_moneda($registro->fej),1,0,'R',0);
	$pdf->Cell($o,$alto,formato_moneda($registro->descuentos),1,0,'R',0);
	$pdf->Cell($p,$alto,formato_moneda($registro->total+$tickets),1,0,'R',0);
	//-----------
	$pdf->Ln();
	$z++;
	}
