...
//-----------------
$quincena = 'VACACIONES DE '.strtoupper($_SESSION['meses_anno'][intval(mes($desde))]).' '.anno($desde);
//-------------	
...
$pdf->SetTitle('Nomina Vacaciones');
...
if ($solicitud>0)
	{
	$pdf->Cell(0,5,'DEFINITIVO NOMINA VACACIONES - LISTADO',0,0,'C'); 
	}
else
	{
	$pdf->Cell(0,5,'PRELIMINAR NOMINA VACACIONES - LISTADO',0,0,'C'); 
	}
$pdf->Ln(10);
...
$pdf->Cell($a=14,5,'Cedula',1,0,'C',1);
$pdf->Cell($b=44,5,'Empleado',1,0,'C',1);
$pdf->Cell($c=15,5,'Ingreso',1,0,'C',1);
$pdf->Cell($d=45,5,'Cargo',1,0,'C',1);
$pdf->Cell($e=17,5,'Sueldo Mensual',1,0,'C',1);
$pdf->Cell($f=17,5,'Prima Ant.',1,0,'C',1);
$pdf->Cell($f,5,'Prima Hijos',1,0,'C',1);
$pdf->Cell($f,5,'Prima Prof.',1,0,'C',1);
$pdf->Cell($g=12,5,'Cant. Dias',1,0,'C',1);
$pdf->Cell($h=20,5,'Bono Vacacional',1,0,'C',1);
$pdf->Cell($t=17,5,'Dias Adic.',1,0,'C',1);
...
while ($registro = $tabla->fetch_object())
{
	...
	$pdf->Cell($f,$alto,formato_moneda($registro->antiguedad),1,0,'R',1);
	$pdf->Cell($f,$alto,formato_moneda($registro->hijos),1,0,'R',1);
	$pdf->Cell($f,$alto,formato_moneda($registro->prof),1,0,'R',1);
	$pdf->Cell($g,$alto,($registro->dias_trabajados),1,0,'C',1);
	$pdf->Cell($h,$alto,formato_moneda($registro->asignaciones-$registro->dias),1,0,'R',1);
	$pdf->Cell($t,$alto,formato_moneda($registro->dias),1,0,'R',1);
	$pdf->Cell($p,$alto,formato_moneda($registro->total),1,0,'R',1);
	//-----------
	$pdf->Ln();
	$z++;
	}
