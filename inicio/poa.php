...
$unidad = $_SESSION["direccion"]; 
//------ MONTAJE DE LOS DATOS
$consultx = "SELECT poa_proyecto_responsable.id as id_proyecto, poa_proyecto_responsable.anno, poa_metas.id, poa_metas.codigo, poa_metas.costo, poa_metas.fecha, poa_metas.meta, poa_metas.actividad, poa_metas.indicador, bn_dependencias.codigo as codigo1 FROM poa_metas, bn_dependencias, poa_proyecto_responsable WHERE bn_dependencias.id = poa_proyecto_responsable.id_direccion AND poa_metas.id_responsable = poa_proyecto_responsable.id AND poa_proyecto_responsable.id_direccion = '$unidad' ";//AND poa_metas.id_proyecto = '$id_proyecto'  
...
<td ><div align="center" ><a data-toggle="tooltip" title="Agregar o Eliminar"><button type="button" class="btn btn-outline-info btn-sm" onclick="programacion('<?php echo ($registro->id); ?>','<?php echo ($registro->anno); ?>','<?php echo ($registro->id_proyecto.'/'.$unidad); ?>');">Balance</button></a></div></td>
...
function programacion(id, anno, unidad)
	...
	$('#modal_xl').load('inicio/poa_programacion.php?id='+id+'&anno='+anno+'&unidad='+unidad);
	}
</script>