...
$num = decriptar($_GET['num']);
$fecha = ($_GET['fecha']);
$anno = anno($_GET['fecha']);

$consultx = "SELECT tipo_orden, concepto as descripcion, numero, fecha, sum(total) as total, estatus, anno FROM credito_adicional_detalle WHERE numero = $num AND YEAR(fecha)='$anno' GROUP BY numero, YEAR(fecha);"; //echo $consultx;

//$id = decriptar($_GET['id']);
//$consultx = "SELECT credito_adicional.id, credito_adicional.tipo_orden, credito_adicional.descripcion, credito_adicional.numero, credito_adicional.fecha, credito_adicional.total, credito_adicional.estatus, credito_adicional.anno FROM credito_adicional WHERE credito_adicional.id = $id LIMIT 1;"; //echo $consultx;
...
$txt="<n><strong>ARTÍCULO 1°.</strong> Se procede a dictar un Crédito Adicional por la cantidad de: <strong>".strtoupper(valorEnLetras($_SESSION['total']))." (Bs. ".formato_moneda($_SESSION['total']).")</strong>, ".ucwords(strtolower($concepto)).".</n>";
$pdf->WriteTag(0,$linea,$txt,0,"J",0,0);
$pdf->Ln(7);
...
//-----------------
$consultx = "SELECT categoria AS codigo, Sum(total) AS total, numero, tipo_orden, a_categoria.descripcion FROM credito_adicional_detalle, a_categoria WHERE numero = $num AND YEAR(fecha)='$anno' AND categoria = a_categoria.codigo GROUP BY numero, categoria ORDER BY categoria;"; //echo consultx;
...
while ($registro = $tablx->fetch_object())
{
	...
	$subtotal = trim($registro->total);
	//------------
	$pdf->SetFont('Times','B',12);
	$txt=$descripcion.": ".formato_categoria($categoria);
	$pdf->Cell(140,6,$txt,1,0,'L',1);
	$txt= formato_moneda($subtotal);
	$pdf->Cell(0,6,$txt,1,0,'R',1);
	$pdf->Ln(6);
	$pdf->SetFont('Times','',12);
	//----------
	$consultx = "SELECT partida AS codigo, Sum(total) AS total, numero, tipo_orden, (a_partidas.descripcion) as descripcion FROM credito_adicional_detalle, a_partidas WHERE credito_adicional_detalle.categoria = '$categoria' AND numero = $num AND YEAR(fecha)='$anno' AND partida = a_partidas.codigo GROUP BY numero, partida ORDER BY categoria, partida;"; //echo $consultx;
	//$consultx = "SELECT a_partidas.codigo, a_partidas.descripcion, credito_adicional_detalle.total FROM credito_adicional_detalle,	a_partidas WHERE credito_adicional_detalle.categoria = '$categoria' AND credito_adicional_detalle.partida = a_partidas.codigo AND credito_adicional_detalle.id_credito = $id;"; //echo $consultx;
...
$pdf->SetFont('Times','B',12);
$pdf->Cell(140,7,'TOTAL A INCREMENTAR………………………………………………………',1,0,'L',1);
...
$txt="<n>Ante todo reciba un cordial saludo Revolucionario Y Socialista, la presente es para solicitar la aprobación y publicación de crédito adicional, por un monto de: <strong>".strtoupper(valorEnLetras($_SESSION['total']))." (Bs. ".formato_moneda($_SESSION['total']).")</strong>, ".ucwords(strtolower($concepto)).".</n>";
$pdf->WriteTag(0,$linea,$txt,0,"J",0,0);
$pdf->Ln(7);
