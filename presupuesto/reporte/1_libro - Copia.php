...
$pdf->Image('../../images/escudo.jpg',70,10,24);
$pdf->Image('../../images/logo_nuevo.jpg',226,10,35);
...
//-----------------
$consulta = "SELECT * FROM a_presupuesto_$anno WHERE categoria = '$categoria' AND codigo = '$partida' LIMIT 1;";
$tablax = $_SESSION['conexionsql']->query($consulta);
$registrox = $tablax->fetch_object();
//-----------------

$pdf->Cell(0,5,'Categoria: '.$registrox->categoria.' Partida: '.$registrox->codigo.' '.$registrox->descripcion,0,0,'C'); $pdf->Ln(2);
...
(SELECT * FROM vista_libro_orden_pago WHERE anno=$anno AND categoria='$categoria' AND partida='$partida' )
UNION (SELECT *,0 FROM vista_libro_tras_sal WHERE anno=$anno AND categoria='$categoria' AND partida='$partida')
UNION (SELECT *,0 FROM vista_libro_tras_ing WHERE anno=$anno AND categoria='$categoria' AND partida='$partida')
UNION (SELECT *,0 FROM vista_libro_nom_asig WHERE anno=$anno AND categoria='$categoria' AND partida='$partida') 
UNION (SELECT *,0 FROM vista_libro_nom_des WHERE anno=$anno AND categoria='$categoria' AND partida='$partida')
UNION (SELECT *,0 FROM vista_libro_decretos WHERE anno=$anno AND categoria='$categoria' AND partida='$partida')) AS tmp ORDER BY fecha;";
//echo $consulta;
$tablax = $_SESSION['conexionsql']->query($consulta);
$registrox = $tablax->fetch_object();
//-----------------
