...
class CrudAdminVehiculos{
    ...
    {
        $query = $this->db->query("SELECT vehiculo.id, vehiculo.id_contribuyente, vehiculo.placa AS numero, vehiculo.marca, vehiculo.modelo, vehiculo.anno, vehiculo.color, contribuyente.rif, contribuyente.nombre FROM vehiculo INNER JOIN contribuyente ON vehiculo.id_contribuyente = contribuyente.id"); 
        //$query->execute();
        $data = array();
    ...
    {
        $query = $this->db->query("SELECT vehiculo.id, vehiculo.placa FROM vehiculo WHERE vehiculo.placa = '$numero'"); 
        //$query->execute();
        $data = array();
        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            $data[] = $row;
    ...
    {
        $query = $this->db->query("SELECT vehiculo.id, vehiculo.id_contribuyente, vehiculo.placa AS numero, vehiculo.marca, vehiculo.modelo, vehiculo.anno, vehiculo.color, contribuyente.rif, contribuyente.nombre FROM vehiculo INNER JOIN contribuyente ON vehiculo.id_contribuyente = contribuyente.id WHERE $filtrar");
        //$query->execute();
        $data = array();
        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            $data[] = $row;
    ...
    public function Eliminar($id)
    {
        ...
        $sql = "DELETE FROM vehiculo WHERE id>1 AND id=?";
        $query = $this->db->prepare($sql);
        $query->bindParam("?", $id);
