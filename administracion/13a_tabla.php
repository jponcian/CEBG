...
<tr>
<td class="TituloTablaP" height="41" colspan="10" align="center">Solicitudes de Pago Registradas</td>
</tr>
<tr>
...
$consultx = "SELECT id_solicitud, tipo_orden, orden.estatus, orden.id, id_contribuyente, orden.rif, fecha, numero, concepto, sum(total) as total1, contribuyente.nombre FROM orden, contribuyente WHERE $filtro tipo_orden='M' AND orden.id_contribuyente = contribuyente.id GROUP BY numero, id_contribuyente ORDER BY fecha DESC, orden.id DESC;"; 
//echo $consultx;
...
<td ><?php if ($registro->estatus==0) { ?><div align="center" ><button type="button" id="boton<?php echo ($registro->id_contribuyente); ?>" class="btn btn-outline-success waves-effect" onclick="generar_solicitud('<?php echo encriptar($registro->id_contribuyente); ?>','boton<?php echo ($registro->id_contribuyente); ?>');" >Generar Solicitud</button></div><?php } ?></td>
<td ><?php if ($registro->estatus==0) { ?><div align="center" ><a data-toggle="tooltip" title="Eliminar la Solicitud"><button type="button" class="btn btn-outline-danger btn-sm" onclick="eliminar_t('<?php echo encriptar($registro->id_contribuyente); ?>');"><i class="fas fa-trash-alt"></i></button></a></div><?php } ?></td>
</tr>
 <?php 
 }
 ?>
