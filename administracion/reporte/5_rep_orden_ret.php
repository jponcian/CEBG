...
$pdf->Cell($a=11,7,'Orden',1,0,'C',1);
$pdf->Cell($b=18,7,'Rif',1,0,'C',1);
$pdf->Cell($c=48,7,'Contribuyente',1,0,'C',1);
$pdf->Cell($d=15,7,'Fecha',1,0,'C',1);
$pdf->Cell($f=26,7,'Total Factura',1,0,'C',1);
$pdf->Cell($f,7,'Iva',1,0,'C',1);
$pdf->Cell($f,7,'Islr',1,0,'C',1);
$pdf->Cell($f,7,'Timbre',1,0,'C',1);
$pdf->Cell($f,7,'Total Ret.',1,0,'C',1);
$pdf->Cell($e=0,7,'Total a Pagar',1,1,'C',1);
$pdf->SetTextColor(0);
...
while ($registro = $tabla->fetch_object())
	...
	if ($registro->descripcion=='A.P.A.R.T.A.D.A')	
		{ $total1=$registro->descripcion; }	
	elseif ($registro->estatus==99)	
		{ $total1='A.N.U.L.A.D.A'; } 
		else { $total1=formato_moneda($registro->total); }
	//----------
	...
	$pdf->Cell($f,5.5,formato_moneda($registro->iva),1,0,'R',1);
	$pdf->Cell($f,5.5,formato_moneda($registro->islr),1,0,'R',1);
	$pdf->Cell($f,5.5,formato_moneda($registro->descuentos-$registro->islr-$registro->iva),1,0,'R',1);
	$pdf->Cell($f,5.5,formato_moneda($registro->descuentos),1,0,'R',1);
	$pdf->Cell($e,5.5,($total1),1,0,'R',1);//$total
	...
	if ($registro->estatus<>99)	
		...
		$otros += $registro->descuentos-$registro->islr-$registro->iva;
		$iva += $registro->iva;
		$islr += $registro->islr;
...
$pdf->Cell($aa+$a+$b+$c+$d/2,7,'TOTALES => ',1,0,'R',1);
$pdf->Cell($d/2+$f,7,formato_moneda($asignaciones),1,0,'R',1);
$pdf->Cell($f,7,formato_moneda($iva),1,0,'R',1);
$pdf->Cell($f,7,formato_moneda($islr),1,0,'R',1);
$pdf->Cell($f,7,formato_moneda($otros),1,0,'R',1);
$pdf->Cell($f,7,formato_moneda($descuentos),1,0,'R',1);
