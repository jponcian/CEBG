...
$pdf->SetFillColor(2, 117, 216);
		if (anno($fecha)<2024)
		{$pdf->Image('../../images/logo_2023.jpg',20,10,28);}
		else
...
$pdf->Cell($f=22,7,'Estatus',1,0,'C',1);
...
while ($registro = $tabla->fetch_object())
	...
	$pdf->Cell($f,5.5,estatus_op($registro->estatus),1,0,'C',1);
	$pdf->Cell($g,5.5,formato_moneda($registro->asignaciones),1,0,'R',1);
	$pdf->Cell($e,5.5,($total),1,0,'R',1);

	$pdf->Ln(5.5);
	if ($registro->estatus<>99)	{$neto += $registro->total;}
	if ($registro->estatus<>99)	{$monto += $registro->asignaciones;}
	//-----------
	$i++;
	}
...
$pdf->Cell(28,6,formato_moneda($monto),1,0,'R',1);
$pdf->Cell(0,6,formato_moneda($neto),1,0,'R',1);
