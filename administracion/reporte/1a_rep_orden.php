...
$pdf->Cell($g=21,7,'Fecha Pago',1,0,'C',1);
$pdf->Cell($e=0,7,'Monto Pagado',1,1,'C',1);
$pdf->SetTextColor(0);
$pdf->SetFillColor(255);
$i=0;
//-----------------
$tabla = $_SESSION['conexionsql']->query($_SESSION['consulta3']);
//-----------------
$i=0; $monto=0;
while ($registro = $tabla->fetch_object())
	...
	elseif ($registro->estatus==99)	{ $total='A.N.U.L.A.D.A'; } else { $total=formato_moneda($registro->monto); }
	//----------
	...
	$pdf->Cell($f,5.5,($_SESSION['tipo_pago'][$registro->tipo_pago]).$registro->num_pago,1,0,'R',1);	$pdf->Cell($g,5.5,voltea_fecha($registro->fecha_pago),1,0,'C',1);
	$pdf->Cell($e,5.5,($total),1,0,'R',1);

	$pdf->Ln(5.5);
	if ($registro->estatus<>99)	{$monto += $registro->monto;}
	//-----------
	$i++;
	}
