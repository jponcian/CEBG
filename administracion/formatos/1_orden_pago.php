...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->Cell(0, 5, 'ORDEN DE PAGO (PATRIA)', 0, 0, 'C');
		//$this->Ln(5);
		...
		$this->Cell($a, 6, utf8_decode('Categoría'), 1, 0, 'C', 1);
		$this->Cell($a + 1, 6, 'Partida', 1, 0, 'C', 1);
		$this->Cell($b = 90 - 9, 6, utf8_decode('Detalle'), 1, 0, 'C', 1);
	...
	function Footer()
	{
		...
		$this->Cell($a = 124, 5, 'RECIB� CONFORME', 1, 0, 'C', 1);
...
if ($pdf->GetY() < $y = 205) {
	...
	$pdf->Cell($a, $y - $pdf->GetY(), '', 1, 0, 'C', 0);
	$pdf->Cell($a + 1, $y - $pdf->GetY(), '', 1, 0, 'C', 0);
	$pdf->Cell($b - 1, $y - $pdf->GetY(), '', 1, 0, 'C', 0);
