...
class PDF extends FPDF
{
	...
	{    
		...
		$consultx = "SELECT	orden.usuario, orden.id, orden.id_contribuyente, orden.rif, orden.fecha, orden.anno, orden.concepto, orden.numero, contribuyente.nombre FROM contribuyente, orden  WHERE orden.estatus=0 AND tipo_orden='F' AND id_contribuyente = $id AND orden.id_contribuyente = contribuyente.id LIMIT 1;"; 
		//echo $consultx;
		...
		//$instituto = instituto();$this->SetFont('Times','I',11.5);
		...
		$this->Cell(0,5,'SOLICITUD ORDEN DE PAGO FINANCIERA',0,0,'C'); 
		$this->Ln(12);
...
//		$this->Ln(6);
		...
		$this->Cell($c=90,6,'Detalle',1,0,'C',1);
		$this->Cell($b=14,6,'Cantidad',1,0,'C',1);
		$this->Cell($d=30,6,'Precio Unitario',1,0,'C',1);	
...
//$consulta = "SELECT orden.id, orden.tipo_orden, orden.id_contribuyente, orden.rif, orden.fecha, orden.anno, orden.concepto, orden.numero, orden.categoria, orden.partida, orden.cantidad, orden.descripcion, orden.precio_uni, orden.asignaciones, orden.descuentos, orden.total, orden.estatus, orden.usuario, orden.fecha_proceso, contribuyente.nombre FROM contribuyente , orden WHERE orden.id_contribuyente = $id  AND orden.estatus=0 AND orden.id_contribuyente = contribuyente.id;";
...
$pdf->SetTitle('Solicitud Orden de Pago Financiera');
...
//-----------------
$consulta = "SELECT orden.categoria, orden.partida, orden.cantidad, orden.descripcion, orden.precio_uni, orden.total FROM orden WHERE tipo_orden='F' AND orden.id_contribuyente = $id  AND orden.estatus=0 ORDER BY id;";
//echo $consulta;
