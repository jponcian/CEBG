...
class PDF extends FPDF
{
	...
	{    
		...
		//------------
		$consultx = "SELECT	orden.estatus, orden.id, orden.id_contribuyente, orden.rif, orden.fecha, orden.anno, orden.concepto, orden.numero, contribuyente.nombre FROM contribuyente, orden  WHERE id_solicitud = $id AND orden.id_contribuyente = contribuyente.id LIMIT 1;"; 
		//echo $consultx;
		...
		$this->Ln(1);
		$this->SetFont('Times','',10);
		$this->Cell($a=0,6,'POR CONCEPTO DE:',1,0,'L',1);
		$this->Ln(6);
...
//-----------------
$consulta = "SELECT orden.categoria, orden.partida, orden.cantidad, orden.descripcion, orden.precio_uni, orden.total, exento FROM orden WHERE orden.id_solicitud = $id ORDER BY descripcion;";
//echo $consulta;
...
if ($_SESSION['estatus']==99)	
	...
	$pdf->Cell(0,5,'ORDEN ANULADA',0,0,'C'); 
	$pdf->SetTextColor(0);
	}
