...
function enviar_datos(id){
	$('#modal_lg').html('<div align="center"><div class="spinner-border" role="status"></div><br><strong>Un momento, por favor...</strong></div>');
	$('#modal_lg').load('administracion/6c_modal.php?id='+id);
}
//---------------------
function guardar(id, tipo_orden)
{
	$('#boton').hide();
	...
	$.ajax({  
		type : 'POST',
		url  : 'administracion/2d_guardar.php?tipo='+tipo_orden,
		dataType:"json",
		data:  parametros, 
...
function buscar(){
	...
	$('#div1').load('administracion/24a_tabla.php?valor='+cambia(document.form1.obuscar.value)+'&tipo='+document.form1.optradio.value+'&fecha1='+document.form1.OFECHA.value+'&fecha2='+document.form1.OFECHA2.value);
}
//------------------
buscar();
</script>