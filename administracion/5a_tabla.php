...
switch ($filtro) {
    case 1:
        $filtrar = " (ordenes_pago.estatus >= 0 and ordenes_pago.estatus <>99) AND ordenes_pago.numero = $dato_buscar AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha ASC, ordenes_pago.numero ASC";
		$_SESSION['titulo'] = "POR CONSULTA (numero: $dato_buscar)";
        break;
    case 3:
        $filtrar = " (ordenes_pago.fecha >= '$fecha1' and ordenes_pago.fecha <='$fecha2') AND (ordenes_pago.estatus >= 0 and ordenes_pago.estatus <>99) AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha ASC, ordenes_pago.numero ASC";
        $_SESSION['titulo'] = "POR FECHA (desde el ".voltea_fecha($fecha1)." al ".voltea_fecha($fecha2).")";
        break;
    case 4:
        $filtrar = " (ordenes_pago.estatus >= 0 and ordenes_pago.estatus <>99) AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha ASC, ordenes_pago.numero ASC";
		$_SESSION['titulo'] = "TODAS";
        break;
    case 5:
        $filtrar = " (ordenes_pago.estatus >= 0 and ordenes_pago.estatus <>99) AND (contribuyente.rif LIKE '%$dato_buscar%' or contribuyente.nombre LIKE '%$dato_buscar%') AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha ASC, ordenes_pago.numero ASC";
        $_SESSION['titulo'] = "POR CONSULTA (contribuyente: $dato_buscar)";
        break;
    case 6:
        //$tabla = ', ordenes_pago_retencion';
		$filtrar = " ordenes_pago.fecha='".date('Y/m/d')."' AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND (ordenes_pago.estatus >= 0 and ordenes_pago.estatus <>99) AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha ASC, ordenes_pago.numero ASC";
		$_SESSION['titulo'] = "POR CONSULTA (dia actual: ".date('d/m/Y').")";
        break;
    case 10:
        $filtrar = " (ordenes_pago.estatus = 0) AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha DESC, ordenes_pago.numero DESC";
        $_SESSION['titulo'] = "POR CONSULTA (pendientes de comprobante)";	
		break;
    case 15:
        $filtrar = " (ordenes_pago.estatus = 10) AND orden_solicitudes.tipo_orden IN ('CC','CD','CP','M','F') AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND contribuyente.id = ordenes_pago.id_contribuyente AND (ordenes_pago.tipo_solicitud <>'NOMINA') GROUP BY ordenes_pago.id ORDER BY ordenes_pago.fecha ASC, ordenes_pago.numero ASC";
...
<td ><div align="center" ><?php if ($registro->estatus>=0 and ($registro->tipo_solicitud=='ORDEN' or $registro->tipo_solicitud=='MANUAL')) { ?><button type="button" data-toggle="modal" data-target="#modal_largo" class="btn btn-info waves-effect" onclick="enviar(<?php echo ($registro->id); ?>);" >Retenciones</button><?php } ?></div></td>
<td ><a data-toggle="tooltip" title="Comprobante de Retencion"><div align="center" ><?php if ($registro->estatus>=0 and ($registro->tipo_solicitud=='ORDEN' or $registro->tipo_solicitud=='MANUAL')) { ?><button type="button" class="btn btn-outline-success waves-effect" onclick="retenciones('<?php echo encriptar($registro->id); ?>');" ><i class="fas fa-print prefix grey-text mr-1"></i></button><?php } ?></div></a></td>
