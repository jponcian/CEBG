...
//---------
$consultx = "SELECT Sum(orden.total) as iva FROM orden , orden_solicitudes , ordenes_pago WHERE ordenes_pago.id= '".$_GET['id']."' AND orden_solicitudes.id_orden_pago = ordenes_pago.id AND orden.id_solicitud = orden_solicitudes.id AND left(orden.partida,7) = '4031801' GROUP BY ordenes_pago.id"; 
//echo $consultx;
$tablx = $_SESSION['conexionsql']->query($consultx);
$registro_x = $tablx->fetch_object();
$iva = $registro_x->iva;
...
$consultx = "SELECT orden.id_solicitud, orden.id, orden.factura, orden.control, orden.fecha_factura, orden_solicitudes.numero, orden_solicitudes.tipo_orden, orden_solicitudes.anno FROM orden, orden_solicitudes WHERE orden_solicitudes.id=orden.id_solicitud AND orden_solicitudes.id_orden_pago='".$_GET['id']."' GROUP BY orden.factura, orden.control;"; //echo $consultx;
$tablx = $_SESSION['conexionsql']->query($consultx);
while ($registro = $tablx->fetch_object())
	{
	$i++;
	$tipo=$registro->tipo_orden.'-'.rellena_cero($registro->numero,3).'-'.$registro->anno;
	?>
<tr >
...
while ($registro = $tablx->fetch_object())
	...
	//--------
	if (substr($registro->partida,0,7)<>'4031801' and (substr($registro->partida,0,3)=='401' or substr($registro->partida,0,3)=='404' or substr($registro->partida,0,3)=='403'))	
		{	$monto_islr += $registro->total; $islr='si'; $cant_islr++; }
...
function calcular()
	...
	if (document.form999.txt_retencion.value==7)
		{
		if (document.form999.txt_porcentaje.value=='' || document.form999.txt_porcentaje.value=='0.1')
		...
		if (document.form999.txt_retencion.value==6)
			...
			if (document.form999.txt_retencion.value==8)
				{ document.form999.txt_porcentaje.value=0.1; 		
				 document.form999.txt_porcentaje2.value=0.1; 		
