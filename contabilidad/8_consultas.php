...
<form id="form1" name="form1" method="post" onsubmit="return evitar();" >
 <div align="center" class="TituloP">Descuentos RRHH</div>
		<br >
<div class="row mb-1 ml-3">
	<strong>Filtro:</strong>
</div>

	<table border="0" class=" ml-6">
  <tbody>
    <tr>
      ...
      <td>
		<select class="custom-select" style="font-size: 14px" name="txt_cta" id="txt_cta" onchange="buscar();">
			...
			<?php
			//--------------------
			$consultx = "SELECT * FROM a_nomina WHERE codigo<>'010' and codigo<>'999' ORDER BY codigo;"; 
...
function imprimir(id, tipo)
	{	
	if (tipo=="FINANCIERA")
		{	window.open("contabilidad/formatos/1b_orden_pago.php?id="+id,"_blank");	}
	if (tipo=="ORDEN" || tipo=="MANUAL")
		{	window.open("contabilidad/formatos/1a_orden_pago.php?id="+id,"_blank");	}
	if (tipo=="NOMINA")
		{	window.open("contabilidad/formatos/1_orden_pago.php?id="+id,"_blank");	}
	}
//---------------------
function imprimir2(id, tipo)
	{	
	if (tipo=="ORDEN" || tipo=="MANUAL")
		{	window.open("contabilidad/formatos/2a_comprobante_pago.php?id="+id,"_blank");	}
	if (tipo=="FINANCIERA")
		{	window.open("contabilidad/formatos/2b_comprobante_pago.php?id="+id,"_blank");	}
	if (tipo=="NOMINA")
		{	window.open("contabilidad/formatos/2_comprobante_pago.php?id="+id,"_blank");	}
	if (tipo=="CHEQUE")
		{	window.open("contabilidad/formatos/3_cheque.php?id="+id,"_blank");	}
	}
//---------------------
...
function rep(tipo)
	...
	if (document.form1.optradio.value>0)
		{
			if (tipo==1)
				{	window.open("contabilidad/reporte/1_rep_orden.php","_blank");	}
			if (tipo==2)
				{	window.open("contabilidad/reporte/5_rep_orden_ret.php","_blank");	}
			if (tipo==3)
				{	window.open("contabilidad/reporte/1a_rep_orden.php","_blank");	}
		}
	}
//----------------
function buscar(){
if (((document.form1.optradio.value==1 || document.form1.optradio.value==2 || document.form1.optradio.value==5 || document.form1.optradio.value==8) && document.form1.obuscar.value!='') || (document.form1.optradio.value==3 && document.form1.OFECHA.value!='' && document.form1.OFECHA2.value!='') || (document.form1.optradio.value==4) || (document.form1.optradio.value==6) || (document.form1.optradio.value==7))
	...
	$('#div1').load('contabilidad/8a_tabla.php?tipo='+document.form1.optradio.value+'&fecha1='+document.form1.OFECHA.value+'&fecha2='+document.form1.OFECHA2.value+'&nomina='+document.form1.txt_cta.value);
	}
}
</script>