...
//		}
	...
	$this->Cell(0,5,'Dirección de Atencion al Ciudadano y Control Social',0,0,'C'); $this->Ln(5);
//	$this->Cell(0,5,'División de Seguridad',0,0,'C'); $this->Ln(5);
	$this->Cell(0,5,'Rif G-20001287-0',0,0,'C'); 
	$this->Ln(8);
	
	$this->SetFont('Times','B',11);
	$this->Cell(0,5,"RELACIÓN DE VISITA DIARIA",0,0,'C'); 
	$this->Ln(7);
	...
	$this->Cell($b=40,7,'Nombres y Apellidos',1,0,'C',1);
	$this->Cell($bb=10,7,'Sexo',1,0,'C',1);
	$this->Cell($b,7,'Instituto',1,0,'C',1);
	$this->Cell($b1=35,7,'Cargo',1,0,'C',1);
	$this->Cell($b2=20,7,'Telefono',1,0,'C',1);
	$this->Cell($f=110,7,'Motivo',1,0,'C',1);
...
while ($registro = $tabla->fetch_object())
	...
	while ($registro1 = $tablx1->fetch_object())
		...
		$j++; 
		if ($j>1) {$texto = $texto . '; ';} 
		$texto = $texto.($registro1->descripcion);
		}
	...
	$y1=$pdf->GetY();
	$x=$pdf->GetX();
	$pdf->SetX(10+$aa+$a+$b+$bb+$c+$b1+$b2);
	//-----------------------------------------MULTICELL
	$pdf->MultiCell($f,5.5, ($texto),0,'J');
	$y2=$pdf->GetY();
	...
	//-------------------
	$pdf->SetFont('Times','',9);
	$pdf->Cell($aa,($alto2),$i+1,1,0,'C',1);
	$pdf->Cell($a,($alto2),$registro->cedula,1,0,'C',1);
	$pdf->SetFont('Times','',7.5);
	$pdf->Cell($b,($alto2),substr($registro->nombre,0,50),1,0,'L',1);
	$pdf->Cell($bb,($alto2),$registro->sexo,1,0,'C',1);
	$pdf->Cell($c,($alto2),$registro->organismo,1,0,'L',1);
	$pdf->Cell($b1,($alto2),$registro->cargo,1,0,'L',1);
	$pdf->Cell($b2,($alto2),$registro->telefono,1,0,'L',1);
	$pdf->Cell($f,($alto2),'',1);
	
	$pdf->SetFont('Times','',9);
	$pdf->Cell($e,($alto2),voltea_fecha($registro->fecha),1,0,'C',1);
	$pdf->Cell($d,($alto2),hora_militar($registro->comienzo),1,0,'C',1);
	$pdf->Cell(0,($alto2),hora_militar($registro->fin),1,0,'C',1);
