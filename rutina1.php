...
//--------------
$consultax = "SELECT fecha FROM rac_historial WHERE fecha=CURDATE() GROUP BY fecha LIMIT 1;";
$tablax = $_SESSION['conexionsql']->query($consultax);
if ($tablax->num_rows > 0) {
} else {
	$consultai = "INSERT INTO rac_historial (fecha, rac, codigo, nomina, cedula, nombre, prof, id_cargo, cargo, sueldo, ubicacion, id_area, id_div, jefe_area, jefe_division) (SELECT CURDATE(), rac.rac, rac.codigo, rac.nomina, rac.cedula, CONCAT(rac.nombre,' ',rac.nombre2,' ',rac.apellido,' ',rac.apellido2) as  nombre, rac.prof, rac.id_cargo, rac.cargo, rac.sueldo, rac.ubicacion, rac.id_area, rac.id_div, rac.jefe_area, rac.jefe_division FROM rac WHERE ubicacion<>'JUBILADOS' AND ubicacion<>'PENSIONADO' AND nomina <> 'EGRESADOS' );";
