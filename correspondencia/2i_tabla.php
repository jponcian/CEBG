...
$consultx = "SELECT cr_memos_ext_destino.id, a_direcciones.direccion, cr_memos_ext_destino.ci_jefe_destino, CONCAT(rac.nombre,' ',rac.nombre2,' ',rac.apellido,' ',rac.apellido2) as nombre FROM cr_memos_ext_destino INNER JOIN a_direcciones ON cr_memos_ext_destino.direccion_destino = a_direcciones.id INNER JOIN rac ON cr_memos_ext_destino.ci_jefe_destino = rac.cedula WHERE	cr_memos_ext_destino.id_correspondencia = '".decriptar($_GET['id'])."';"; 
//echo $consultx;
