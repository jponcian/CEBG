...
function direccion(id){
	...
	$('#modal_lg').load('correspondencia/7g_modal.php?id='+id);
	}
//---------------------
function borrar(id)
	{
		...
		if (result.isConfirmed) {
			...
			var parametros = "id=" + id;
					$.ajax({
					url: "correspondencia/7g_eliminar.php",
					type: "POST",
					data: parametros,
					success: function(r) {
...
function editar(id)
	{
	$('#modal_lg').html('<div align="center"><div class="spinner-border" role="status"></div><br><strong>Un momento, por favor...</strong></div>');
	$('#modal_lg').load('correspondencia/7b_modal.php?id='+id);
	}
//--------------------- PARA BUSCAR
function listar_bienes(){
	...
	$('#div3').load('correspondencia/7d_tabla.php?id='+cambia(document.form999.txt_numero.value)+'&nombre='+cambia(document.form999.txt_bien.value)+'&origen='+(document.form999.txt_origen.value));
}
//------------------
function enviar_memo(id)
	...
	function()
		...
		$.ajax({  
			type : 'POST',
			url  : 'correspondencia/7k_guardar.php?',
			dataType:"json",
			data:  parametros, 
...
function generar_memo(id, id_origen, anno)
	{
		...
		if (result.isConfirmed) {
			...
			$.ajax({  
				type : 'POST',
				url  : 'correspondencia/7j_guardar.php?',
				dataType:"json",
				data:  parametros, 
				success:function(data) {  	
					if (data.tipo=="info")
...
function guardar(id)
	{
	 if (validar_detalle()==0)
		...
		function()
				...
				$.ajax({
				url: "correspondencia/7f_guardar.php",
				type: "POST",
				dataType:"json",
				data: parametros,
				success: function(data) {  	
...
function combo()
{
	$.ajax({
        type: "POST",
        url: 'correspondencia/7c_combo.php?origen='+document.form999.txt_origen.value+'&id='+document.form999.oid.value,
...
function buscar2(){
	...
	$('#div1').load('correspondencia/7a_tabla.php?valor='+cambia(document.form1.obuscar.value)+'&tipo=3');
}
//----------------
function buscar(){
if((document.form1.obuscar.value=="  " || document.form1.obuscar.value==" " || document.form1.obuscar.value=="") && document.form1.optradio.value<3){}
		...
		$('#div1').html('<div align="center"><div class="spinner-border" role="status"></div><br><strong>Un momento, por favor...</strong></div>');
		$('#div1').load('correspondencia/7a_tabla.php?valor='+cambia(document.form1.obuscar.value)+'&tipo='+document.form1.optradio.value+'&fecha1='+document.form1.OFECHA.value+'&fecha2='+document.form1.OFECHA2.value);
		}
}
//-----------------------
function agregar()
	{
	$('#modal_lg').html('<div align="center"><div class="spinner-border" role="status"></div><br><strong>Un momento, por favor...</strong></div>');
	$('#modal_lg').load('correspondencia/7b_modal.php');
	}
//---------------------
