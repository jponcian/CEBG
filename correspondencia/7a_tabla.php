...
elseif ($_GET['tipo']=='2')	 
	{
	$filtro = " (asunto like '%".($_GET['valor'])."%' or cuerpo like '%".($_GET['valor'])."%') AND estatus=10 AND ";	
	}
	elseif ($_GET['tipo']=='3')	 
		{	
		$filtro = " estatus=10 AND ";	
		}
		elseif ($_GET['tipo']=='4')	 
			{	
			$filtro = " estatus=10 AND cr_memos_div_destino.estatus_recepcion=0 AND ";	
			}
			elseif ($_GET['tipo']=='5')	 
				{	
				$filtro = " estatus=10 AND cr_memos_div_destino.estatus_recepcion=10 AND ";	
				}
				elseif ($_GET['tipo']=='8')	 
					{	
					$fecha1 = voltea_fecha($_GET['fecha1']);
					$fecha2 = voltea_fecha($_GET['fecha2']);
					$filtro = " estatus=10 AND fecha>='$fecha1' AND fecha<='$fecha2' AND ";
					}
?>
